function labels = logistic_regression(Xtrain,Ytrain,Xtest,stepsize,iterations)
    ...
    extra = ones(size(Xtrain,1),1);
    %Add new column
    Xtrain = [extra Xtrain];
    
    [weights, err] = gradient_ascent_decay(Xtrain,Ytrain,stepsize,iterations);
    
    extra = ones(size(Xtest,1),1);
    %Add new column
    Xtest = [extra Xtest];
    
    exponent = weights'* Xtest'; 
    labels = 1 ./ ( 1 + exp(-exponent'));
    labels(labels>0.5) = 1;
    labels(labels<=0.5) = 0;
end